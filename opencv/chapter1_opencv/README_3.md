...
## 📂 파일 정보
*   **파일명**: `3.py`
*   **사용된 주요 함수**: `cv.imread()`, `cv.namedWindow()`, `cv.setMouseCallback()`, `cv.rectangle()`, `cv.imshow()`, `cv.imwrite()`, `cv.waitKey()`
...
1. **상태 관리 변수 초기화**
   * 드래그 여부(`is_dragging`), 시작 좌표(`x_start, y_start`), 그리고 추출된 영역(`roi`)을 저장할 변수들을 선언합니다.

2. **마우스 이벤트 처리 (`on_mouse`)**
   * **LBUTTONDOWN**: 클릭 지점을 시작점으로 기록하고 드래그 상태를 활성화합니다.
   * **MOUSEMOVE**: 드래그 중일 때 실시간으로 `cv.rectangle()`을 사용하여 선택 영역을 초록색 사각형으로 표시합니다.
   * **LBUTTONUP**: 마우스를 떼는 순간의 좌표를 끝점으로 기록하고, Numpy 슬라이싱을 이용해 해당 영역을 잘라냅니다.
